"""Pytest fixtures for job_profile skills feature tests."""
import uuid
import asyncio
import asyncpg
import pytest
...
def app():
    ...
    from app.features.job_profile.skills.router import router as skills_router
...
def authenticated_client(app):
    """Yields (client, user_data, profile_id, job_profile_id) for an authenticated user."""
    async def _setup():
        conn = await asyncpg.connect(settings.database_url)
        try:
            await ensure_auth_schema(conn)
            email = f"jp-skills-test-{uuid.uuid4().hex}@example.com"
