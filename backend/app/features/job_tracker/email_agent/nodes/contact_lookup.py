...
from app.features.job_tracker.email_agent.state import EmailAgentState, ApolloResult
...
async def contact_lookup(state: EmailAgentState) -> dict:
    """
    ...
    try:
        ...
        return _not_found_result("apollo", state.get("errors", []) + errors, state.get("debug_log", []) + debug_log, "timeout")
        ...
        return _not_found_result("apollo", state.get("errors", []) + errors, state.get("debug_log", []) + debug_log, str(exc))
    ...
    if not people:
        ...
        return _not_found_result("apollo", state.get("errors", []) + errors, state.get("debug_log", []) + debug_log, "no results")
    ...
    if not matched_people:
        ...
        return _not_found_result("apollo", state.get("errors", []) + errors, state.get("debug_log", []) + debug_log, "company mismatch")
