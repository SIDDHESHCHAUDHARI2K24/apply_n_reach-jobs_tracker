...
async def email_generator(state: EmailAgentState) -> dict:
    """
    ...
    if parsed_jd is None:
        ...
        return {"generated_emails": [], "errors": state.get("errors", []) + errors, "debug_log": state.get("debug_log", []) + debug_log}
    ...
    try:
        email_body = await _chat(system=system_prompt, user=user_message, max_tokens=600)
...
def _build_user_message(state: EmailAgentState, parsed_jd: dict) -> str:
    parts: list[str] = []
...
def _collect_signals(state: EmailAgentState) -> list[str]:
    signals: list[str] = []
