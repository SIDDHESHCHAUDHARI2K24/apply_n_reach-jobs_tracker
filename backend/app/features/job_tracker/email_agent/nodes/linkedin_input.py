...
NOTE: raw_linkedin_paste is not in the base EmailAgentState schema because
it is only populated when the user provides LinkedIn context (optional step
in the wizard). The graph only routes here when:
  (a) the user explicitly provided a LinkedIn paste in the UI, OR
  (b) Apollo found a person but no verified email (fallback branch).
...
manual description) into a structured LinkedInContext object using the LLM.
...
from app.features.job_tracker.email_agent.state import EmailAgentState, LinkedInContext
...
async def linkedin_input(state: EmailAgentState) -> dict:
    """
    ...
    try:
        raw_json = await _chat(system=_SYSTEM, user=raw_paste, max_tokens=512)
