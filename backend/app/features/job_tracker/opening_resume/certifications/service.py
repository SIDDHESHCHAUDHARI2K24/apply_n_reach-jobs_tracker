"""Service functions for job_opening_certifications section."""
import asyncpg
...
async def list_entries(
    ...
    return await conn.fetch(
        "SELECT * FROM job_opening_certifications WHERE resume_id=$1 ORDER BY display_order ASC",
        resume_id,
    )
...
async def get_entry(
    ...
    row = await conn.fetchrow(
        "SELECT * FROM job_opening_certifications WHERE id=$1 AND resume_id=$2",
        entry_id,
        resume_id,
    )
    if not row:
...
async def delete_entry(
    ...
    result = await conn.execute(
        "DELETE FROM job_opening_certifications WHERE id=$1 AND resume_id=$2",
        entry_id,
        resume_id,
    )
