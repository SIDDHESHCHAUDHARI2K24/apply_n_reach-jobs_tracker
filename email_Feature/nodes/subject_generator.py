...
from state import OutreachState, SubjectLineSet, FollowUpDraft
...
def subject_generator(state: OutreachState) -> dict:
    """
    ...
    try:
        raw_json = chat(system=_SUBJECT_SYSTEM, user=context, max_tokens=256)
        ...
        # Fallback subject lines that are at least specific
        subject_options = [
    ...
    followup_body: str = ""
    try:
        followup_body = chat(system=_FOLLOWUP_SYSTEM, user=context, max_tokens=300)
    except Exception as exc:
