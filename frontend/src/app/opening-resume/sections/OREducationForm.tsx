...
function toForm(item: OREducation | null): FormFields {
  return {
...
function fromForm(form: FormFields): Omit<OREducation, 'id' | 'resume_id' | 'display_order'> {
  return {
...
interface Props {
  initial: OREducation | null
  isSaving: boolean
  onSave: (data: Omit<OREducation, 'id' | 'resume_id' | 'display_order'>) => Promise<void>
  onCancel: () => void
}
...
export function OREducationForm({ initial, isSaving, onSave, onCancel }: Props) {
