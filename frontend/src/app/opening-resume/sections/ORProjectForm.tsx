...
function toForm(item: ORProject | null): FormFields {
  return {
    title: item?.title ?? '',
...
function fromForm(form: FormFields): Omit<ORProject, 'id' | 'resume_id'> {
  return {
    title: form.title,
...
interface Props {
  initial: ORProject | null
  isSaving: boolean
  onSave: (data: Omit<ORProject, 'id' | 'resume_id'>) => Promise<void>
  onCancel: () => void
}
...
export function ORProjectForm({ initial, isSaving, onSave, onCancel }: Props) {
