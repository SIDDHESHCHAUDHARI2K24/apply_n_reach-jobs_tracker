...
function toForm(item: JPEducation | null): FormFields {
  return {
...
function fromForm(form: FormFields): Omit<JPEducation, 'id' | 'job_profile_id'> {
  return {
...
interface Props {
  initial: JPEducation | null
  isSaving: boolean
  onSave: (data: Omit<JPEducation, 'id' | 'job_profile_id'>) => Promise<void>
  onCancel: () => void
}
...
export function JPEducationForm({ initial, isSaving, onSave, onCancel }: Props) {
