...
import { useJPEducation } from './useJPEducation'
import { JPEducationForm } from './JPEducationForm'
...
export function JPEducationList({ jobProfileId }: Props) {
  const { items, isLoading, error, load, create, update, remove, importAll } = useJPEducation(jobProfileId)
  const [showForm, setShowForm] = useState(false)
  const [editing, setEditing] = useState<JPEducation | null>(null)
  ...
  async function handleCreate(data: Omit<JPEducation, 'id' | 'job_profile_id'>) {
    try {
      setIsSaving(true)
      await create(data)
      setShowForm(false)
  ...
  async function handleUpdate(data: Omit<JPEducation, 'id' | 'job_profile_id'>) {
    if (!editing) return
    try {
      setIsSaving(true)
  ...
  return (
    <div className="space-y-4">
      ...
      {items.length === 0 && !showForm && (
        <p className="text-sm text-slate-400 py-4">No education entries yet.</p>
      )}
      ...
      {showForm ? (
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          <JPEducationForm initial={null} isSaving={isSaving} onSave={handleCreate} onCancel={() => setShowForm(false)} />
        </div>
      ) : (
        <button
