...
function toForm(item: JPExperience | null): FormFields {
  return {
...
function fromForm(form: FormFields): Omit<JPExperience, 'id' | 'job_profile_id'> {
  return {
...
interface Props {
  initial: JPExperience | null
  isSaving: boolean
  onSave: (data: Omit<JPExperience, 'id' | 'job_profile_id'>) => Promise<void>
  onCancel: () => void
}
...
export function JPExperienceForm({ initial, isSaving, onSave, onCancel }: Props) {
