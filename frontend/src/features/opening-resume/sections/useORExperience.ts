...
export function useORExperience(openingId: string) {
  const [items, setItems] = useState<ORExperience[]>([])
  ...
  }, [openingId, loadExperience])

  const create = useCallback(async (data: Omit<ORExperience, 'id' | 'resume_id'>) => {
    setIsSaving(true); setError(null)
    try {
      const item = await openingResumeApi.createExperience(openingId, data)
  ...
  const update = useCallback(async (id: string, data: Partial<Omit<ORExperience, 'id' | 'resume_id'>>) => {
    setIsSaving(true); setError(null)
    try {
      const item = await openingResumeApi.updateExperience(openingId, id, data)
  ...
  const remove = useCallback(async (id: string) => {
    setIsSaving(true); setError(null)
    try {
      await openingResumeApi.deleteExperience(openingId, id)
