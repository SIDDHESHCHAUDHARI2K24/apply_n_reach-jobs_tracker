...
export function useCertifications() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const items = await profileApi.getCertifications()
  ...
  const create = useCallback(async (data: Omit<Certification, 'id' | 'profile_id' | 'created_at' | 'updated_at'>) => {
    setState(s => ({ ...s, isSaving: true, error: null }))
    try {
      const item = await profileApi.createCertification(data)
  ...
  const update = useCallback(async (id: string, data: Partial<Omit<Certification, 'id' | 'profile_id' | 'created_at' | 'updated_at'>>) => {
    setState(s => ({ ...s, isSaving: true, error: null }))
    try {
      const item = await profileApi.updateCertification(id, data)
