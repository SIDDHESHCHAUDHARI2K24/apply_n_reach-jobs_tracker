...
export function useEducation() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const items = await profileApi.getEducation()
  ...
  const create = useCallback(async (data: Omit<Education, 'id' | 'profile_id' | 'created_at' | 'updated_at'>) => {
    setState(s => ({ ...s, isSaving: true, error: null }))
    try {
      const item = await profileApi.createEducation(data)
  ...
  const update = useCallback(async (id: string, data: Partial<Omit<Education, 'id' | 'profile_id' | 'created_at' | 'updated_at'>>) => {
    setState(s => ({ ...s, isSaving: true, error: null }))
    try {
      const item = await profileApi.updateEducation(id, data)
