...
export function useResearch() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const items = await profileApi.getResearch()
  ...
  const create = useCallback(async (data: Omit<Research, 'id' | 'profile_id' | 'created_at' | 'updated_at'>) => {
    setState(s => ({ ...s, isSaving: true, error: null }))
    try {
      const item = await profileApi.createResearch(data)
  ...
  const update = useCallback(async (id: string, data: Partial<Omit<Research, 'id' | 'profile_id' | 'created_at' | 'updated_at'>>) => {
    setState(s => ({ ...s, isSaving: true, error: null }))
    try {
      const item = await profileApi.updateResearch(id, data)
  ...
  const remove = useCallback(async (id: string) => {
    setState(s => ({ ...s, isSaving: true, error: null }))
    try {
      await profileApi.deleteResearch(id)
